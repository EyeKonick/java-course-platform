...
import { ChevronRight, FileCode, BookMarked, X } from 'lucide-react';
      ...
      <aside className={[
        ...
        <nav className="flex-1 overflow-y-auto py-2">
          {curriculum.chapters.map((chapter, chIdx) => {
            ...
            const displayTitle = chapter.title.replace(/^Part \d+:\s*/, '');
            return (
              <div key={chapter.id} className="mb-1">
                ...
                >
                  <ChevronRight
                    className="w-3.5 h-3.5 text-slate-500 shrink-0 transition-transform duration-300"
                    style={{ transform: chapterOpen ? 'rotate(90deg)' : 'rotate(0deg)' }}
                  />
                ...
                {/* Animated accordion panel — CSS grid trick */}
                <div
                  style={{
                    display: 'grid',
                    gridTemplateRows: chapterOpen ? '1fr' : '0fr',
